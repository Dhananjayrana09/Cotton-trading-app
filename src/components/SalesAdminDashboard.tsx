...
const SalesAdminDashboard: React.FC = () => {
  ...
  const handleConfirmSale = async (order: SalesOrder) => {
    ...
    try {
      // Call n8n workflow for sales confirmation
      const response = await fetch(`${import.meta.env.VITE_N8N_WEBHOOK_URL}/sales-confirmation`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          confirmed_by: 'admin', // In production, this would be the actual admin user ID
          action: 'confirm_sale'
        }),
      });
      ...
      if (response.ok) {
        ...
        alert('Sale confirmed successfully! Contract generation initiated.');
      } else {
