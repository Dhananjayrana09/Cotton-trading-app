...
const CottonDataTable: React.FC<CottonDataTableProps> = ({ data, onEdit, onView }) => {
  ...
  const getStatusIcon = (status: CottonTradeData['status'] | undefined) => {
    switch (status) {
      case 'approved':
  ...
  const getStatusBadgeClass = (status: CottonTradeData['status'] | undefined) => {
    switch (status) {
      case 'approved':
              ...
              sortedData.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-gray-900">{item.indent_number || 'N/A'}</div>
                    <div className="text-sm text-gray-500">{item.crop_year || 'N/A'}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-gray-900">{item.buyer_name || 'Unknown'}</div>
                    <div className="text-sm text-gray-500">{item.buyer_type || 'N/A'}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">{item.variety || 'N/A'}</div>
                    <div className="text-sm text-gray-500">{item.fibre_length || 'N/A'}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {item.bales_quantity?.toLocaleString() || '0'}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">₹{item.offer_price?.toLocaleString() || '0'}</div>
                    <div className="text-sm text-gray-500">Bid: ₹{item.bid_price?.toLocaleString() || '0'}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <div className="w-16 bg-gray-200 rounded-full h-2 mr-2">
                        <div 
                          ...
                          style={{ width: `${item.parsing_confidence || 0}%` }}
                        ></div>
                      </div>
                      <span className="text-sm text-gray-600">{item.parsing_confidence || 0}%</span>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
