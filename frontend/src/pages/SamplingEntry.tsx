...
const SamplingEntry: React.FC = () => {
  ...
  const handleAllocationFound = useCallback((allocation: AllocationData) => {
    setAllocationData(allocation);
    setCalculation(null);
    setSubmissionResult(null);
  }, []);

  const handleCalculationComplete = useCallback((calc: SamplingCalculation) => {
    setCalculation(calc);
  }, []);

  const handleSubmissionComplete = useCallback((success: boolean, message: string) => {
